export default function TicketReducer(state,action){
    switch(action.type){
        ...
        case "UPDATE_TICKET":
            return{
                ...state,
                tickets:state.tickets.map((ticket) =>ticket.id === action.payload.id ? action.payload : ticket),editingTicket:null
            }
        case "DELETE_TICKET":
        ...
        if(state.editingTicket && state.editingTicket === action.payload.id){
            return{
                ...state,
                tickets:state.tickets.filter((ticket) => ticket.id !== action.payload.id),editingTicket:null
            }
